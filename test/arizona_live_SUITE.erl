...
init_per_suite(Config) ->
    ...
    % Create mock request adapter (replaces arizona_cowboy_request for standalone testing)
    MockAdapterModule = arizona_live_mock_adapter,
    ...
    """, [{module, merl:term(MockAdapterModule)}]),

    % Compile and load mock modules
    {ok, _AdapterBinary} = merl:compile_and_load(MockAdapterCode),
...
mock_request() ->
    arizona_request:new(arizona_live_mock_adapter, #{}, #{
